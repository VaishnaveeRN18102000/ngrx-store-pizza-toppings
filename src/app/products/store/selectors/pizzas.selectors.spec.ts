...
describe('Pizzas Selectors', () => {
  ...
  describe('getSelectedPizza', () => {
    it('should return selected pizza as an entity', () => {
      ...
      store.select(fromRoot.getRouterState).subscribe(routerState => (params = routerState.state.params));

      expect(params).toEqual({ pizzaId: '2' });

      store.select(fromSelectors.getSelectedPizza).subscribe(selectedPizza => (result = selectedPizza));
  ...
  describe('getPizzaVisualised', () => {
    it('should return selected pizza composed with selected toppings', () => {
      ...
      store.select(fromSelectors.getPizzaVisualised).subscribe(selectedPizza => (result = selectedPizza));
